package com.jinmgr.m3u8.download;

import com.jinmgr.m3u8.Exception.M3u8Exception;
import com.jinmgr.m3u8.utils.Constant;
import com.jinmgr.m3u8.utils.EchoUtil;
import com.jinmgr.m3u8.utils.MediaFormat;
import com.jinmgr.utils.FFmpegUtils;
import com.jinmgr.utils.HttpUtils;
...
public class M3u8DownloadFactory {
    ...
    public static class M3u8Download {
        ...
        private File startDownload() {
            ...
            try {
                boolean flag = fixedThreadPool.awaitTermination(20, TimeUnit.MINUTES);
                if (flag && downloadComp) {
                    ...
                    //开始合并视频
                    targetFile = new File(dir + FILESEPARATOR + fileName + ".mp4");
                    mergeTs(targetFile);
                    //删除多余的ts片段
                    deleteFiles();
                } else {
         ...
         * 合并下载好的ts片段
         */
        private void mergeTs(File targetFile) {
            try {
                File file = new File(targetFile.getAbsolutePath() + "sub");
                System.gc();
                ...
                //转码
                FFmpegUtils.getInstance().formatMedia(file.getAbsolutePath(), true);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
...
//                logger.info(urls + "下载完毕！\t已完成" + finishedCount + "个，还剩" + (tsSet.size() - finishedCount) + "个！");
        ...
        private StringBuilder getUrlContent(String urls, boolean isKey) {
            ...
            if (count > retryCount) throw new M3u8Exception(lastError);
            return content;
        }
        ...
        private byte[] decrypt(byte[] sSrc, int length, String sKey, String iv, String method) throws Exception {
            ...
            byte[] ivByte;
            if (iv.startsWith("0x")) ivByte = EchoUtil.hexStringToByteArray(iv.substring(2));
    ...
    public static M3u8Download getInstance() {
        if (m3u8Download == null) {
            synchronized (M3u8Download.class) {
                if (m3u8Download == null) m3u8Download = new M3u8Download();
            }
        }
        return m3u8Download;
    }
